						  ...
						  if($data['file_type']  == 'folder'){
                            ...
                            <tr id="<?php echo $istar ?>">
                                <?php if (!FM_READONLY): ?>
                                    <td class="custom-checkbox-td">
                                        <div class="custom-control custom-checkbox">
                                            <input type="checkbox" class="custom-control-input" id="<?php echo $istar ?>" name="file[]" value="<?php echo $data['Id'] ?>">
                                            <label class="custom-control-label" for="<?php echo $istar ?>"></label>
                                        </div>
                                    </td><?php endif; ?>
                                    <td data-sort=<?php echo fm_convert_win(fm_enc($f)) ?>>
                                        <div class="filename"><a href="javascript:void(0);"><span class="thumb"><i class="<?php echo $img ?>"></i></span> <?php echo fm_convert_win(fm_enc($data['file_name'])) ?>
							...
							$filelink = '?action=filemanager&p=' . urlencode($data['root_path']) . '&amp;view=' . urlencode($data['file_name']);
							   ?>
                                ...
                                <td data-sort=<?php echo fm_enc($data['file_name']) ?>>
                                    <div class="filename">
                                    <?php
                                    ...
                                    <li><a class="dropdown-item" title="<?php echo lng('Download') ?>" href="?action=filemanager&p=<?php echo urlencode($data['root_path']) ?>&amp;dl=<?php echo urlencode($data['file_name']) ?>"><?php echo lng('Download') ?></a></li>
                                        <?php if (!FM_READONLY): ?>
                                           ...
                                           <!-- <li><a href="#" onclick="checkit('<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>')"><span>Starred</span></a>-->
											...
											<li><a class="dropdown-item" title="<?php echo lng('Unstarred') ?>" href="javascript:void(0)" onclick="unstarrred('<?php echo $data['Id'] ?>')"> <?php echo lng('Unstarred') ?></a></li>
                                            <li><hr class="dropdown-divider" /></li>
											<li><a class="dropdown-item" href="#" onclick="sendtotrash('<?php echo urlencode(trim($data['root_path'] . '/' . $data['file_name'], '/')) ?>', <?php echo $istar ?>)"><span>Delete</span></a></li>
										<?php endif; ?>
                                    </ul>
                    ...
                    <div class="col-12">
                        <ul class="list-inline footer-action m-0">
                            ...
                            <li class="list-inline-item"><input type="submit" class="hidden" name="delete" id="a-delete" value="Delete">
                                <a href="javascript:void(0);" class="btn btn-small btn-outline-primary btn-2" onclick="massstarreddelete('<?php echo FM_ROOT_PATH ?>', '<?php echo urlencode(FM_PATH) ?>')"><i class="fa fa-trash"></i> <?php echo lng('Delete') ?> </a></li>
...
function starred_preview_show(div_id){
	$("#starred_preview_div"+div_id).show();
	$("#main-table2").hide();
	$("#starred_preview_foot").hide();
}

function starred_preview_div_close(div_id){
	$("#starred_preview_div"+div_id).hide();
	$("#main-table2").show();
	$("#starred_preview_foot").show();
}
