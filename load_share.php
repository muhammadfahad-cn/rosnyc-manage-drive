...
$num_folders = count($folders);
   // Show shared Files
    $msg='';
	$sucessmsg='';
   if(isset($_GET['shareddel'])){
		$result=$con->query("DELETE from filemanager_share WHERE Id=".$_GET['shareddel']);
	    $sucessmsg="shared";
	}
	//if (isset($_GET['shared']) && !FM_READONLY) {
		//echo "SELECT * from filemanager_share WHERE share_emails='".$_SESSION['userEmail']."' OR userId=".$_SESSION['userId'];
		$result=$con->query("SELECT * from filemanager_share WHERE share_emails='".$_SESSION['userEmail']."' OR userId=".$_SESSION['userId']);

		$result_file=$con->query("SELECT * from filemanager_share WHERE share_emails='".$_SESSION['userEmail']."' OR userId=".$_SESSION['userId']." AND file_type='file'");
	    $row_file_cnt = $result_file->num_rows;

		$result_folder=$con->query("SELECT * from filemanager_share WHERE share_emails='".$_SESSION['userEmail']."' OR userId=".$_SESSION['userId']." AND file_type='folder'");
...
.dz-custom {
	  ...
	  <div class="d-flex flex-wrap">
		   ...
		   <form action="" method="post" class="w-100" novalidate>
                ...
                <div class="m-table-responsive">
                    <table class="table table-hover table-sm <?php echo $tableTheme; ?>" id="main-table1">
						...
						while($data=mysqli_fetch_assoc($result)){
							if($_SESSION['userId'] == $data['to_user'] ){
								 $sharedpath=$path.'shared';
								 $FM_IMAGE_URL=FM_IMAGE_URL.'shared/';
							}
							else {
								$sharedpath=$path;
								$FM_IMAGE_URL=FM_IMAGE_URL;
							}

						 if($data['file_type']  == 'folder'){
						    $is_link = is_link($sharedpath . '/' . $f);
                            $img = $is_link ? 'icon-link_folder' : 'fa fa-folder-o';
                            $modif_raw = filemtime($sharedpath . '/' . $data['file_name']);
                            $modif = date(FM_DATETIME_FORMAT, $modif_raw);
                            if ($calc_folder) {
                                $filesize_raw = fm_get_directorysize($sharedpath . '/' . $data['file_name']);
                            ...
                            $perms = substr(decoct(fileperms($sharedpath . '/' . $f)), -4);
                            if (function_exists('posix_getpwuid') && function_exists('posix_getgrgid')) {
                                $owner = posix_getpwuid(fileowner($sharedpath . '/' . $f));
                                $group = posix_getgrgid(filegroup($sharedpath . '/' . $f));
                            } else {
                                $owner = array('name' => '?');
                                $group = array('name' => '?');
                            }
                            ...
                            <tr>
                                <?php if (!FM_READONLY): ?>
                                    <td class="custom-checkbox-td">
                                        <div class="custom-control custom-checkbox">
                                            <input type="checkbox" class="custom-control-input" id="<?php echo $ii ?>" name="file[]" value="<?php echo $data['id'] ?>">
                                            <label class="custom-control-label" for="<?php echo $ii ?>"></label>
                                        </div>
                                    </td><?php endif; ?>
                                    <td data-sort=<?php echo fm_convert_win(fm_enc($data['file_name'])); ?>>
                                        <div class="filename">
										<a onclick="insidefoldershared('<?php echo urlencode(trim(($data['root_path'] != '' ? '/' .$data['root_path'] : '') . '/' . $data['file_name'], '/')) ?>', '<?php echo base64_encode($data['userId']) ?>')">
                                        <span class="thumb"><i class="<?php echo $img ?>"></i></span> <?php echo fm_convert_win(fm_enc($data['file_name'])) ?></a>
											<?php echo($is_link ? ' &rarr; <i>' . readlink($sharedpath . '/' . $f) . '</i>' : '') ?></div>
                                    </td>
                                ...
                                <td class="inline-actions">
                                    <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                                <!--<li> <a class="dropdown-item" title="<?php echo lng('DirectLink')?>" onclick="insidefolder('<?php echo urlencode(trim(FM_PATH . '/' . $data['file_name'], '/')) ?>')" href="javascript:void(0)"><?php echo lng('DirectLink')?></a><li>-->
                                                <?php if (!FM_READONLY): ?>
												...
												<?php
												 if($_SESSION['userId'] == $data['to_user'] ){ } else {
												?>
												<li><hr class="dropdown-divider" /></li>
												<li><a class="dropdown-item" title="<?php echo lng('Delete') ?>" href="javascript:void(0)" onclick="sharedelete('<?php echo $data['id'] ?>')"> <?php echo lng('Delete') ?></a></li>
                                                 <?php } ?>
											<?php endif; ?>
                                           </ul>
                                    </div>
							...
							$is_link = is_link($sharedpath . ($data['root_path'] != '' ? '/' . $data['root_path'] : '') .'/'.$data['file_name']);
							$img = $is_link ? 'fa fa-file-text-o' : fm_get_file_icon_class($sharedpath . '/' . $data['file_name']);

							$modif_raw = filemtime($sharedpath . ($data['root_path'] != '' ? '/' . $data['root_path'] : '') .'/'.$data['file_name']);
                            $modif = date(FM_DATETIME_FORMAT, $modif_raw);
                            $filesize_raw = fm_get_size($sharedpath . ($data['root_path'] != '' ? '/' . $data['root_path'] : '') .'/'.$data['file_name']);
                            $filesize = fm_get_filesize($filesize_raw);
							if($_SESSION['userId'] == $data['to_user'] ){
							$filelink = '?action=filemanager&p=' . urlencode('shared/'.$data['root_path']) . '&amp;view=' . urlencode($data['file_name']);
							}
							else {
							...
							?>
							 <tr>
                                <?php if (!FM_READONLY): ?>
                                    <td class="custom-checkbox-td">
                                    <div class="custom-control custom-checkbox">
                                        <input type="checkbox" class="custom-control-input checkvalues" id="checkvalues" name="file[]" value="<?php echo $data['id'] ?>">
                                    ...
                                    if (in_array(strtolower(pathinfo($data['file_name'], PATHINFO_EXTENSION)), array('gif', 'jpg', 'jpeg', 'png', 'bmp', 'ico', 'svg', 'webp', 'avif'))): ?>
                                            ...
                                            <a href="#" onclick="share_preview_show('<?php echo $ii; ?>')" data-preview-image="<?php echo $imagePreview ?>" title="<?php echo fm_enc($data['file_name']) ?>"><span onclick="share_preview_show('<?php echo $ii; ?>')"><img src="<?php echo $imagePreview ?>" alt="img"></span><?php  echo fm_convert_win(fm_enc($data['file_name']))?>
									<?php else: ?>
                                            <a href="#" onclick="share_preview_show('<?php echo $ii; ?>')"><span onclick="share_preview_show('<?php echo $ii; ?>')" ><i class="<?php echo $img ?>" style="font-size: 48px; padding-left: 12px;"></i></span><?php echo fm_convert_win(fm_enc($data['file_name'])) ?>
                                    <?php endif; ?>
                                            </span></a>
                                    <?php echo($is_link ? ' &rarr; <i>' . readlink($sharedpath . '/' . $f) . '</i>' : '') ?>
                                    </div>
                                </td>
                                ...
                                <td class="inline-actions">
                                <div class="dropdown">
                                    <a class="font-size-16 text-muted" href="#" role="button" id="starredDropdownMenuLink" data-mdb-toggle="dropdown" aria-expanded="false" > <i class="fa fa-ellipsis-v"></i>
                                   </a>
                                    <ul class="dropdown-menu" aria-labelledby="starredDropdownMenuLink">
                                        <li> <a class="dropdown-item" title="<?php echo lng('Preview') ?>" href="#" onclick="share_preview_show('<?php echo $ii; ?>')" data-toggle="lightbox" data-gallery="tiny-gallery" data-title="<?php echo fm_convert_win(fm_enc($f)) ?>" data-max-width="100%" data-width="100%"><?php echo lng('Preview') ?></a></li>
  ...
  <div id="shared_preview_div<?php echo $ii; ?>" style="display:none;">
  <?php //if (isset($_GET['view'])) {
    ...
    if($data['to_user'] > 0){
		...
		$file_url = fm_enc($FM_IMAGE_URL.($data['root_path'] != '' ? '/' . $data['root_path'] : '').'/'.$data['file_name']);
    ...
    <div class="filemanager-wrap flex-wrap">
	  <div><a href="#" onclick="shared_preview_div_close('<?php echo $ii; ?>')">Close</a></div>
        <div class="row ">
            <div class="col-12">
      ...
      <!--</div>
                ...
                <!-- Code for the Show the Image/doc files in center -->
                        ...
                        if (empty($folders) && empty($files)) {
							...
							//fm_get_filesize(@disk_free_space($sharedpath))
                            ?>
                            <tfoot>
                                <tr><?php if (!FM_READONLY): ?>
                                    ...
                                    <td class="gray" colspan="<?php echo (!FM_IS_WIN && !$hide_Cols) ? '6' : '4' ?>">
                                        ...
                                        <?php echo lng('PartitionSize').': <span class="badge badge-light">'.$totalsizegbArr[1].'GB</span> '.lng('FreeOf').' <span class="badge badge-light">1 GB</span>'; //fm_get_filesize(@disk_total_space($sharedpath)) ?>
                                    </td>
                                </tr>
                            </tfoot>
                            <?php
                ...
                <div class="row" id="shared_preview_foot">
                    <?php if (!FM_READONLY): ?>
                    <div class="col-12">
                        <ul class="list-inline footer-action m-0">
                            ...
                            <li class="list-inline-item"><input type="submit" class="hidden" name="delete" id="a-delete" value="Delete">
                                <a href="javascript:void(0);" class="btn btn-small btn-outline-primary btn-2" onclick="masssharedelete('<?php echo FM_ROOT_PATH ?>', '<?php echo urlencode(FM_PATH) ?>')"><i class="fa fa-trash"></i> <?php echo lng('Delete') ?> </a></li>
...
?>
<script>
function share_preview_show(div_id){
	$("#shared_preview_div"+div_id).show();
	$("#main-table1").hide();
	$("#shared_preview_foot").hide();
}

function shared_preview_div_close(div_id){
	$("#shared_preview_div"+div_id).hide();
	$("#main-table1").show();
	$("#shared_preview_foot").show();
}
</script>